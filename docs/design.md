...
Ghostty reads `~/.config/ghostty/config` on both macOS and Linux. Because the same path works on every supported platform, the config is symlinked directly like the other `~/.config/` tools — no per-machine fallback is needed, unlike Powerlevel10k.

**macOS pitfall — a second config path overrides this one.** On macOS Ghostty *also* loads `~/Library/Application Support/com.mitchellh.ghostty/config`, and it loads that file *after* the XDG one, so any value set there silently wins (Ghostty auto-creates a template there on first launch if no config exists, which is how a stale override once shadowed half of this repo's settings — theme, opacity, padding). The dual-path order is Ghostty's intended "shared base + macOS overrides" mechanism, but this repo deliberately does not use it: the App Support file is deleted (Ghostty only regenerates its template there when *no* config exists anywhere, and the XDG symlink always exists) and every setting lives in the shared XDG file. If a file ever reappears at that path, it will silently shadow the dotfiles config — delete it. Per-OS conditionals are not needed because Ghostty's config has none anyway — instead, `macos-*` keys are inert on Linux, missing fallback fonts are skipped, and `background-blur` is a no-op where unsupported, so macOS-flavored settings are safe in the shared file.

The config largely mirrors `config/kitty` behavior (MesloLGS NF font, 4px padding, block cursor, copy-on-select), with deliberate divergences: the theme is **Catppuccin Mocha** (kitty stays Dracula) and `background-opacity` is 0.8 (kitty uses 0.75) — both referenced as built-in Ghostty themes/values rather than vendored, since Ghostty ships the iTerm2 color-scheme collection — and `font-size` is 13 (kitty uses 12; 13 is also Ghostty's macOS default, but it is set explicitly because Ghostty's Linux default is 12, so both platforms render the same size). One deliberate non-default: `copy-on-select = clipboard` (not the `true`/`primary` default) so selections reach the system clipboard and `ctrl+v` pastes them, matching kitty's `copy_on_select yes` across both Linux and macOS. macOS niceties carried over from the old App Support config: `macos-option-as-alt` (Option as Meta for zsh word motions), Korean fallback font `Apple SD Gothic Neo` plus `JetBrainsMono Nerd Font` for glyph coverage, `background-blur`, and `alt+left/right=unbind` so Ghostty's default esc+b/esc+f word jumps don't swallow Zellij's alt navigation.

**SSH to hosts without Ghostty's terminfo.** Ghostty sets `TERM=xterm-ghostty`, which resolves locally via the `TERMINFO` path inside the app bundle but does not exist in the stock terminfo database of remote hosts without Ghostty installed (macOS ships no `xterm-ghostty` entry in `/usr/share/terminfo`), breaking keys and colors over SSH. The config enables `shell-integration-features = ssh-env,ssh-terminfo` (Ghostty ≥ 1.2): when `ssh` is run from inside Ghostty, `ssh-terminfo` first tries to install Ghostty's terminfo on the remote via `tic`, and `ssh-env` otherwise falls back to `TERM=xterm-256color` for that session only — local sessions keep full `xterm-ghostty` capabilities. Listing these two features does not disturb the defaults (`cursor`, `title`, `path`), which stay enabled. kitty needs no equivalent because `kitty.conf` pins `term xterm-256color` globally.
